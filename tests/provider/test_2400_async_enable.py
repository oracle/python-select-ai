...
async def setup_and_teardown(request, async_connect, provider_params):
    ...
    cls.user_prefix = f"P2400A_{uuid.uuid4().hex[:8].upper()}"
    cls.db_users = []
    cls.missing_users = [
...
class TestAsyncEnableProvider:
    ...
    async def test_2402(self):
        ...
        with pytest.raises(
            oracledb.DatabaseError, match=r"ORA-46238"
        ) as exc_info:
            await select_ai.async_grant_http_access(
                users=db_users,
                provider_endpoint=self.provider_endpoint,
        ...
        assert self.missing_users[0] in str(exc_info.value)
    ...
    async def test_2405(self):
        ...
        try:
            ...
            logger.info(
                "Provider enabled successfully with string user input."
            )
        except Exception as exc:
            pytest.fail(
    ...
    async def test_2407(self):
        ...
        with pytest.raises(
            oracledb.DatabaseError, match=r"ORA-29261"
        ) as exc_info:
            await select_ai.async_grant_http_access(
                users=self.db_users,
