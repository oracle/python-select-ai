...
@pytest.fixture(scope="class", autouse=True)
def setup_and_teardown(request, connect, delete_vec_params):
...
class TestDeleteVectorIndex:
    ...
    def create_credential(cls, genai_cred=None, objstore_cred=None):
        ...
        if params.get("cred_username") and params.get("cred_password"):
            ...
            try:
                logger.info(
                    f"Creating ObjectStore credential: {objstore_cred}"
                )
                select_ai.create_credential(
    ...
    def delete_profile(cls, profile):
        ...
        try:
            profile.delete()
            logger.info(
                f"Profile '{profile.profile_name}' deleted successfully."
            )
        except Exception as e:
    ...
    def delete_and_wait(self, force=True, pattern=".*", wait_seconds=1):
        ...
        for idx in all_indexes:
            try:
                self.delete_index_with_retry(idx, force=force)
                logger.info(f"Deleted index: {idx.index_name}")
                time.sleep(wait_seconds)
            except Exception as e:
                logger.warning(
                    f"Warning: failed to delete index {idx.index_name}: {e}"
                )
        remaining = list(self.vecidx.list(index_name_pattern=pattern))
        logger.info(
            f"Remaining indexes after delete: {[i.index_name for i in remaining]}"
        )
    ...
    def teardown_method(self, method):
        ...
        try:
            ...
            logger.info(
                f"Vector index '{self.index_name}' deleted successfully."
            )
        except Exception as e:
    ...
    def assert_index_count(self, pattern, expected):
        ...
        logger.info(
            f"Indexes matching '{pattern}': {[i.index_name for i in actual]}"
        )
        assert (
            len(actual) == expected
    ...
    def test_5101(self):
        ...
        self.assert_index_count(f"^{self.index_name}", 1)
    ...
    def test_5104(self):
        ...
        logger.info(
            f"Verified vector table '{table_name}' removed: {count==0}"
        )
        assert count == 0
    ...
    def test_5105(self):
        ...
        with pytest.raises(oracledb.DatabaseError) as exc_info:
            self.vector_index.create(replace=True)
        logger.info(
            "Expected DatabaseError on recreate: %s",
            exc_info.value,
        )
        assert "ORA-00955" in str(exc_info.value)
        self.verify_and_cleanup_vectab(self.index_name)
    ...
    def test_5110(self):
        ...
        try:
            self.vector_index.set_attribute(
                attribute_name="match_limit", attribute_value=10
            )
        except Exception as e:
            logger.error(f"set_attribute() raised {e} unexpectedly.")
            pytest.fail(f"set_attribute() raised {e} unexpectedly.")
        logger.info("Deleting index after setting attributes")
        self.vector_index.delete(force=True)
        time.sleep(1)
        actual_indexes = list(
            self.vector_index.list(index_name_pattern=f"^{self.index_name}$")
        )
    ...
    def test_5117(self):
        ...
        logger.info("Deleting index with exact test pattern")
        self.vector_index.create(replace=True)
        self.vector_index.delete(force=True)
        time.sleep(1)
        actual = list(
    ...
    def test_5121(self):
        ...
        assert "ORA-00955" in str(exc_info.value)
        logger.info(
            "Expected recreate failure confirmed: %s",
            exc_info.value,
        )
