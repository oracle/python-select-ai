...
async def vector_index_test_state(request):
    ...
    logger.info("Vector index '%s' created for test.", request.cls.index_name)
    yield
    logger.info("TearDown for %s", request.function.__name__)
    try:
        await request.cls.delete_index_with_retry(
            request.cls.async_vector_index,
            force=True,
        )
        logger.info(
...
class TestAsyncDeleteVectorIndex:
    ...
    async def delete_and_wait(cls, force=True, pattern=".*", wait_seconds=1):
        ...
        all_indexes = [
            index
            async for index in cls.vecidx.list(index_name_pattern=pattern)
        ]
        if not all_indexes:
        ...
        for index in all_indexes:
            try:
                await cls.delete_index_with_retry(index, force=force)
    ...
    async def assert_index_count(self, pattern, expected):
        actual = [
            index
            async for index in self.vecidx.list(index_name_pattern=pattern)
        ]
        logger.info(
    ...
    async def test_5101(self):
        ...
        await self.assert_index_count(f"^{self.index_name}", 1)
    ...
    async def test_5105(self):
        ...
        await self.verify_and_cleanup_vectab(self.index_name)
    ...
    async def test_5110(self):
        ...
        actual_indexes = [
            ...
            async for index in self.async_vector_index.list(
                index_name_pattern=f"^{self.index_name}$"
            )
        ]
    ...
    async def test_5114(self):
        ...
        await self.delete_index_with_retry(self.async_vector_index, force=True)
        await asyncio.sleep(1)
        actual_indexes = [
            index
            async for index in self.async_vector_index.list(
    ...
    async def test_5121(self):
        ...
        with pytest.raises(oracledb.DatabaseError) as exc_info:
            await self.async_vector_index.create(replace=False)
        assert "ORA-00955" in str(exc_info.value)
        logger.info("Expected recreate failure confirmed: %s", exc_info.value)
