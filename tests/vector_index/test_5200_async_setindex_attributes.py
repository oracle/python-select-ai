...
async def vector_index_state(request):
    ...
    indexes = [
        idx
        async for idx in AsyncVectorIndex.list(
...
class TestAsyncSetVectorIndexAttributes:
    ...
    @classmethod
    async def create_credential(cls, genai_cred=None, objstore_cred=None):
    ...
    async def test_5208(self):
        ...
        temp_profile = await self.create_profile(
    ...
    async def test_5209(self):
        ...
        attrs = await self.async_vector_index.get_attributes()
        logger.info("Reset VectorIndex profile to default: %s", attrs.__dict__)
        assert attrs.profile_name == self.profile.profile_name
    ...
    async def test_5210(self):
        ...
        logger.info(
            "Testing deleted profile behavior via AsyncVectorIndex fetch..."
        )
        temp_profile_name = self.set_vec_params["temp_profile_name"]
        temp_profile = await self.create_profile(
            profile_name=temp_profile_name
    ...
    async def test_5212(self):
        ...
        try:
            ...
            raise AssertionError(
                "create_credential() raised an unexpected exception: " f"{exc}"
            )
        ...
        attrs = await self.async_vector_index.get_attributes()
        logger.info(
            "Updated credential: %s", attrs.object_storage_credential_name
        )
        assert (
    ...
    async def test_5214(self):
        ...
        logger.info(
            "Testing missing credential behavior via AsyncVectorIndex create..."
        )
        ...
        await select_ai.async_delete_credential(
            temp_credential_name, force=True
        )
