...
@pytest.fixture(scope="class", autouse=True)
def setup_and_teardown(request, connect, enabledisable_params):
    ...
    with select_ai.cursor() as cursor:
        cursor.execute(
            "begin execute immediate 'drop table test_items purge'; exception when others then null; end;"
        )
        cursor.execute(
            "create table test_items (id number primary key, name varchar2(50))"
        )
...
class TestEnableDisableVectorIndex:
    ...
    def test_5502(self):
        ...
        self.vector_index.disable()
        logger.info(
            f"Attempting second disable of vector index: {self.index_name}"
        )
        self.vector_index.disable()
    ...
    def test_5505(self):
        ...
        logger.info(
            f"Disabling vector index: {self.index_name} to test query blocking"
        )
        self.vector_index.disable()
    ...
    def test_5509(self):
        ...
        logger.info(
            f"Disabling vector index: {self.index_name} to check pipeline inactivity"
        )
        self.vector_index.disable()
        ...
        with select_ai.cursor() as cursor:
            ...
            if row is None:
                logger.info(
                    f"Pipeline is inactive (no entry in user_cloud_pipelines)"
                )
                assert True
                return
            status_table = row[0]
            logger.info(
                f"Status table found: {status_table}, querying should fail"
            )
            with pytest.raises(oracledb.DatabaseError):
                cursor.execute(
    ...
    def test_5510(self):
        ...
        with select_ai.cursor() as cursor:
            status_table = self.wait_for_pipeline_entry(cursor, pipeline_name)
            assert (
