...
def setup_and_teardown(
    ...
    request.cls.index_name = p["attr_index_name"]
...
class TestSetVectorIndexAttributes:
    ...
    @staticmethod
    def stop_pipeline_if_exists(vector_index):
        try:
            pipeline_name = vector_index.get_attributes().pipeline_name
    ...
    def delete_index_with_retry(
        ...
        cls.stop_pipeline_if_exists(vector_index)
        for attempt in range(retries):
            try:
                vector_index.delete(include_data=include_data, force=force)
                return
            except oracledb.DatabaseError as exc:
    ...
    def test_5202(self):
        ...
        attrs = self.vector_index.get_attributes()
        logger.info(
            f"Updated similarity_threshold: {attrs.similarity_threshold}"
        )
    ...
    def test_5204(self):
        ...
        logger.info(
            "Testing repeated update of the same attribute 'similarity_threshold'..."
        )
        self.vector_index.set_attribute("similarity_threshold", 0.8)
        self.vector_index.set_attribute("similarity_threshold", 0.5)
        attrs = self.vector_index.get_attributes()
        logger.info(
            f"Final similarity_threshold value: {attrs.similarity_threshold}"
        )
    ...
    def test_5206(self):
        ...
        logger.info(
            f"Set match_limit to {min_limit}, fetching attributes for verification..."
        )
        attrs = self.vector_index.get_attributes()
    ...
    def test_5208(self):
        ...
        temp_profile_name = self.set_vec_params["temp_profile_name"]
        ...
        logger.info(
            f"Set profile_name to {temp_profile_name}, fetching attributes..."
        )
        attrs = self.vector_index.get_attributes()
        logger.info(
            f"VectorIndex attributes after profile update: {attrs.__dict__}"
        )
        assert attrs.profile_name == temp_profile_name
        vecidx = VectorIndex()
        vec_index = (list(vecidx.list(index_name_pattern=self.index_name)))[0]
        logger.info(
            f"Persisted VectorIndex after profile update: {vec_index.__dict__}"
        )
        assert attrs.profile_name == vec_index.profile.profile_name
        logger.info(
            "Persisted VectorIndex reflects updated profile correctly."
        )
    ...
    def test_5209(self):
        ...
        self.vector_index.set_attribute("profile_name", temp_profile_name)
        logger.info(
            f"Set profile_name to {temp_profile_name}, verifying update..."
        )
        attrs = self.vector_index.get_attributes()
        assert attrs.profile_name == temp_profile_name
        vecidx = VectorIndex()
    ...
    def test_5210(self):
        ...
        logger.info(
            "Testing deleted profile behavior via VectorIndex fetch..."
        )
        temp_profile_name = self.set_vec_params["temp_profile_name"]
        temp_profile = self.create_profile(profile_name=temp_profile_name)
        self.vector_index.set_attribute("profile_name", temp_profile_name)
    ...
    def test_5212(self):
        ...
        logger.info(
            "Testing update of object_storage_credential_name with pipeline handling..."
        )
        attrs = self.vector_index.get_attributes()
        ...
        objstore_credential = self.get_cred_param(
            self.set_vec_params["temp_objstore_cred"]
        )
        logger.info(
        ...
        try:
            ...
            raise AssertionError(
                f"create_credential() raised an unexpected exception: {e}"
            )
        ...
        attrs = self.vector_index.get_attributes()
        logger.info(
            f"Updated credential: {attrs.object_storage_credential_name}"
        )
        assert (
            attrs.object_storage_credential_name
            == self.set_vec_params["temp_objstore_cred"]
        )
        logger.info("Deleting temporary credential: TEMP_OBJSTORE_CRED")
        try:
            select_ai.delete_credential(
                self.set_vec_params["temp_objstore_cred"], force=True
            )
            logger.info("TEMP_OBJSTORE_CRED deleted successfully.")
        except Exception as e:
            pytest.fail(
                f"delete_credential() raised unexpected exception: {e}"
            )
        logger.info(
            "Restoring original Object Store credential: OBJSTORE_CRED"
        )
        self.vector_index.set_attribute(
    ...
    def test_5213(self):
        ...
        logger.info(
            "Testing update of object_storage_credential_name with delete handling..."
        )
        attrs = self.vector_index.get_attributes()
        pipeline_name = attrs.pipeline_name
        logger.info(f"Retrieved pipeline name: {pipeline_name}")
        ...
        logger.info(
            f"Credential updated to: {attrs.object_storage_credential_name}"
        )
        logger.info("Deleting temporary credential: TEMP_OBJSTORE_CRED")
        try:
            select_ai.delete_credential(
        ...
        self.get_cred_param(self.objstore_cred)
        select_ai.create_credential(
            credential=self.get_cred_param(self.objstore_cred),
            replace=True,
        )
        self.vector_index.set_attribute(
            "object_storage_credential_name", self.objstore_cred
        ...
        assert attrs.object_storage_credential_name == self.objstore_cred
        logger.info(
            "Object Store credential restoration after delete verified successfully."
        )

    def test_5214(self):
        """Deleted credential leaves VectorIndex unusable until restored."""
        logger.info(
            "Testing missing credential behavior via VectorIndex create..."
        )
        temp_credential_name = self.set_vec_params["temp_objstore_cred"]
        attrs = self.vector_index.get_attributes()
        pipeline_name = attrs.pipeline_name
        ...
        failing_index = VectorIndex(
            index_name=f"{self.index_name}_missing_cred",
    ...
    def test_5216(self):
        ...
        logger.info(
            "Testing update of description attribute (should raise DatabaseError)..."
        )
        with pytest.raises(oracledb.DatabaseError) as exc_info:
            self.vector_index.set_attribute(
    ...
    def test_5218(self):
        ...
        logger.info(
            "chunk_size update prevented successfully; original value verified."
        )
    ...
    def test_5219(self):
        ...
        original_chunk_overlap = (
            self.vector_index.get_attributes().chunk_overlap
        )
        with pytest.raises(oracledb.DatabaseError) as exc_info:
        ...
        logger.info(
            "chunk_overlap update prevented successfully; original value verified."
        )

    def test_5220(self):
        """Update vector_distance_metric (should fail)."""
        logger.info(
            "Testing update of vector_distance_metric (should fail with ORA-20047)..."
        )
    ...
    def test_5224(self):
        ...
        logger.info(
            f"Attributes after attempted profile object update: {attrs.__dict__}"
        )
        try:
            self.delete_profile(temp_profile)
            logger.info(
                f"Temporary profile '{temp_profile_name}' deleted successfully."
            )
        except Exception as e:
    ...
    def test_5228(self):
        ...
        logger.info(
            "Testing update with invalid enum value for vector_distance_metric..."
        )
        with pytest.raises(oracledb.DatabaseError):
            self.vector_index.set_attribute(
    ...
    def test_5232(self):
        ...
        logger.info(
            "Testing update with invalid attributes object (non-object input)..."
        )
        with pytest.raises(AttributeError):
    ...
    def test_5233(self):
        ...
        connect_kwargs = self.reconnect_params.copy()
    ...
    def test_5235(self):
        ...
        attrs = self.vector_index.get_attributes()
        logger.info(
            f"Final match_limit value after concurrent updates: {attrs.match_limit}"
        )
    ...
    def test_5238(self):
        ...
        logger.info(
            "Testing attribute update after deleting and recreating the vector index..."
        )
        self.delete_index_with_retry(self.vector_index, force=True)
