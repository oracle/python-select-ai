...
FROM C##CLOUD$SERVICE.ALL_CLOUD_AI_PROFILE_ATTRIBUTES
...
FROM C##CLOUD$SERVICE.ALL_CLOUD_AI_PROFILES
WHERE profile_name = :profile_name
AND owner = COALESCE(:owner, SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA'))
"""
...
FROM C##CLOUD$SERVICE.ALL_CLOUD_VECTOR_INDEXES v
...
FROM C##CLOUD$SERVICE.ALL_CLOUD_VECTOR_INDEXES
...
SELECT attribute_name, attribute_value
FROM C##CLOUD$SERVICE.ALL_CLOUD_VECTOR_INDEX_ATTRIBUTES
WHERE index_name = :index_name
AND owner = COALESCE(:owner, SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA'))
"""
...
SELECT conversation_id,
       ...
       description,
       retention_days
from C##CLOUD$SERVICE.USER_CLOUD_AI_CONVERSATIONS
"""
...
SELECT conversation_prompt_id,
       ...
       sid,
       serial#
FROM C##CLOUD$SERVICE.USER_CLOUD_AI_CONVERSATION_PROMPTS
WHERE conversation_id = :conversation_id
ORDER BY created
"""
...
FROM C##CLOUD$SERVICE.USER_CLOUD_PIPELINES
